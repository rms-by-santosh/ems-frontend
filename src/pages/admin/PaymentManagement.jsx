...
export default function PaymentManagement() {
  ...
  const handleAddPayment = async (e) => {
    ...
    try {
      await axios.post(
        ...
        { ...addForm, applicant: selectedApplicant._id },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      setAddForm({ date: "", reference: "", amount: "", method: "" });
      setShowAdd(false);
      fetchPayments(selectedApplicant._id); // Refresh payments for this applicant
    } catch (err) {
      setError("Error: " + getErrorMsg(err));
    }
  };
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      await axios.put(
        `${import.meta.env.VITE_API_URL}/payments/${editId}`,
        { ...editForm },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      setEditId(null);
      fetchPayments(selectedApplicant._id);
    } catch (err) {
      setError("Error: " + getErrorMsg(err));
    }
  };
              ...
              applicantsToShow.map((app, idx) => (
                <tr key={app._id || idx}>
                  ...
                  <td className="actions">
                    ...
                    <button className="btn-add" onClick={() => { setSelectedApplicant(app); setShowAdd(true); setEditId(null); }}>Add Payment</button>
                  </td>
                </tr>
              ))
            )}
